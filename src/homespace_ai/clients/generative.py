...
class DisabledGenerativeClient(BaseGenerativeClient):
    ...
    async def generate_answer(
        ...
        raise GenerationUnavailableError("Generative provider is disabled.")
...
class GeminiGenerativeClient(BaseGenerativeClient):
    ...
    async def generate_answer(
        ...
        if not self.model:
            raise GenerationUnavailableError("Generation model is not configured.")
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    ...
                    if not candidates:
                        raise GenerationUnavailableError("Gemini returned empty candidates.")

                    parts = candidates[0].get("content", {}).get("parts", [])
                    answer = "".join(part.get("text", "") for part in parts)
                    if not answer.strip():
                        raise GenerationUnavailableError("Gemini returned an empty answer.")
                    return answer.strip()
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code != 200:
                    ...
                    raise GenerationUnavailableError(f"Groq API returned status {response.status_code}.")

                data = response.json()
                choices = data.get("choices", [])
                if not choices:
                    raise GenerationUnavailableError("Groq returned empty choices.")
                
                answer = choices[0].get("message", {}).get("content", "")
                if not answer.strip():
                    raise GenerationUnavailableError("Groq returned an empty answer.")
                return answer.strip()
        except httpx.TimeoutException:
